...
### Roadmap (next up)
- **Interface audit by design-systems-architect + the-artist agents** — Spawn both agents to review the current interface design (upload screen, grid, frame preview, theatrical view, settings) and propose specific improvements. Designer focuses on layout, hierarchy, consistency, type system, color palette; artist focuses on overall feel, vibe, emotional resonance, what's missing or trying-too-hard. Synthesize findings into an action list before implementing.
